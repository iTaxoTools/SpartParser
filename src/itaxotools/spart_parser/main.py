...
class Spart:
            ...
            for spNum in range(1, numSpartitions+1):
                ...
                if checkKey(self.spartDict['spartitions'][str(spNum)], 'label'):
                    ...
                    if checkKey(self.spartDict['spartitions'][str(spNum)], 'spartitionScore'):
                        ...
                        if not self.spartDict['spartitions'][str(spNum)]['spartitionScore'] is None:
                ...
                if checkKey(self.spartDict['spartitions'][str(spNum)], 'subsetScoreType'):
                    hasSubset_score_type = True
                    if self.spartDict['spartitions'][str(spNum)]['subsetScoreType']:
                        subset_score_type += self.spartDict['spartitions'][str(spNum)]['subsetScoreType'] +' / '
                else:
                    if hasSubset_score_type:
                        subset_score_type += '? / '
                #spartition_score_type
                if checkKey(self.spartDict['spartitions'][str(spNum)], 'spartitionScoreType'):
                    hasSpartition_score_type = True
                    if self.spartDict['spartitions'][str(spNum)]['spartitionScoreType']:
                        spartition_score_type += self.spartDict['spartitions'][str(spNum)]['spartitionScoreType'] +' / '
                else:
                ...
                #individual_score_type
                # spartition_score_type
                if checkKey(self.spartDict['spartitions'][str(spNum)], 'individualScoreType'):
                    hasIndividual_score_type = True
                    if self.spartDict['spartitions'][str(spNum)]['individualScoreType']:
                        individual_score_type += self.spartDict['spartitions'][str(spNum)]['individualScoreType'] + ' / '
                else:
    ...
    def addSpartition(self, label: str, remarks: str = None, **kwargs) -> None:
        ...
        is passed as keyword arguments."""
        spartitionsTags = {'spartition_score': 'spartitionScore',
                           'spartition_score_type': 'spartitionScoreType',
                           'individual_score_type': 'individualScoreType',
                           'subset_score_type': 'subsetScoreType',
                           'evidenceType': 'evidenceDataType', 
                           'evidenceDiscriminationType': 'evidenceDiscriminationDataType'
                           }
        ...
        for spNum in range(1, spartitionNumber):
            if checkKey(self.spartDict['spartitions'][str(spNum)], 'spartitionScore'):
                self.spartDict['spartitions'][str(spartitionNumber)]['spartitionScore'] = None
    ...
    def addConcordance(self, spartition: str, evidenceName: str, analysis: str, date: str, **kwargs) -> None:
        ...
        concordanceTags = {
            ...
            'evidenceDataType':'evidenceDataType',
            'evidenceDiscriminationType':'evidenceDiscriminationType',
            'evidenceDiscriminationDataType':'evidenceDiscriminationDataType',
            'evidenceSignificanceTest':'evidenceSignificanceTest'
            }
        ...
        for spartitionRemark in self.spartDict['spartitions'].keys():
            for spartitionName in self.spartDict['spartitions'][spartitionRemark].keys():
                if checkKey(self.spartDict['spartitions'][spartitionRemark], 'label') and not spartition == self.spartDict['spartitions'][spartitionRemark]['label']:
                    continue
                if not spartitionName == 'concordances':
                    continue
                self.spartDict['spartitions'][spartitionRemark]['concordances'][evidenceName] = {}
                self.spartDict['spartitions'][spartitionRemark]['concordances'][evidenceName]['analysis'] = analysis
                self.spartDict['spartitions'][spartitionRemark]['concordances'][evidenceName]['date'] = date

                for concordances in self.spartDict['spartitions'][spartitionRemark]['concordances'].keys():
                    for k, v in kwargs.items():
                        if k in concordanceTags:
                            k = concordanceTags[k]
                            self.spartDict['spartitions'][spartitionRemark]['concordances'][concordances][k] = v




    def addConcordantLimit(self, spartitionLabel: str, evidenceName: str, subsetA: str, subsetB: str,**kwargs) -> None:
        spartition = self.getSpartitionFromLabel(spartitionLabel)
        if not checkKey(spartition['concordances'][evidenceName], 'concordantLimit'):
            spartition['concordances'][evidenceName]['concordantLimit'] = {}
        limitKey = str(subsetA) + '_' + str(subsetB)
        spartition['concordances'][evidenceName]['concordantLimit'][limitKey] = {}
        spartition['concordances'][evidenceName]['concordantLimit'][limitKey] = kwargs
    ...
    def getIndividualLatLon(self, id: str) -> tuple[float, float] or None:
        ...
        if checkKey(self.spartDict['individuals'], id):
            ...
            if not locality:
                return None
        if checkKey(self.spartDict['locations'], locality):
    ...
    def getSpartitionConcordances(self, evidenceName: str) -> list[str]:
        """Returns a list with the evidenceNames of all concordances of the given spartition"""
    ...
    def getConcordanceData(self, spartition: str, concordance: str) -> dict[str, object]:
        concordanceData = {}
        for spartitionName in self.spartDict['spartitions'].keys():
            if not self.spartDict['spartitions'][spartitionName]['label'] == spartition:
                continue
            for concordanceLabel in self.spartDict['spartitions'][spartitionName]['concordances'].keys():
                if concordanceLabel == concordance:
                    for tag, val in self.spartDict['spartitions'][spartitionName]['concordances'][concordanceLabel].items():
                        if tag not in ['concordantLimit']:
                            concordanceData[tag] = val
        return concordanceData
    
    def getAnalysis(self, spartition: str, concordance: str) -> str:
        return self.getSpartitionFromLabel(spartition)['concordances'][concordance].get('analysis')

    def getConcDate(self, spartition: str, concordance: str) -> date:
        return self.getSpartitionFromLabel(spartition)['concordances'][concordance].get('date')


    def getEvidenceType(self, spartition: str, concordance: str) -> str:
        return self.getSpartitionFromLabel(spartition)['concordances'][concordance].get('evidenceType')

    def getEvidenceDataType(self, spartition: str, concordance: str) -> str:
        return self.getSpartitionFromLabel(spartition)['concordances'][concordance].get('evidenceDataType')

    def getEvidenceDiscriminationType(self, spartition: str, concordance: str) -> str:
        return self.getSpartitionFromLabel(spartition)['concordances'][concordance].get('evidenceDiscriminationType')

    def getEvidenceDiscriminationDataType(self, spartition: str, concordance: str) -> str:
        return self.getSpartitionFromLabel(spartition)['concordances'][concordance].get('evidenceDiscriminationDataType')




    def getConcordantLimits(self, spartitionLabel: str, evidenceName: str) -> list[str]:
        """Returns a list with all concordantLimits of the given concordance"""
        concordantLimits_list = []

        for spartition in self.spartDict['spartitions'].keys():
            for tag in self.spartDict['spartitions'][spartition].keys():
                if tag == 'label':
                    if self.spartDict['spartitions'][spartition][tag] == spartitionLabel:
                        for key, val in self.spartDict['spartitions'][spartition]['concordances'][evidenceName].items():
                            if key =='concordantLimit':
                                for limit in self.spartDict['spartitions'][spartition]['concordances'][evidenceName]['concordantLimit'].keys():
                                    concordantLimits_list.append(limit)
        return concordantLimits_list


    def getConcordantLimitData(self, spartition: str, concordance: str, limitKey: str) -> dict[str, object]:
        for spartitionName in self.spartDict['spartitions'].keys():
            if self.spartDict['spartitions'][spartitionName]['label'] != spartition:
                continue
            return self.spartDict['spartitions'][spartitionName]['concordances'][concordance]['concordantLimit'][limitKey]
        raise Exception('No data present')

    def getSubsetNumberA(self, spartition: str, concordance: str, limitKey: str) -> int:
        x = limitKey.split('_')
        return x[0]

    def getSubsetNumberB(self, spartition: str, concordance: str, limitKey: str) -> int:
        x = limitKey.split('_')
        return x[1]

    def getnumIndividualsInSubsetA(self, spartition: str, concordance: str, limitKey: str) -> int:
        return self.getSpartitionFromLabel(spartition)['concordances'][concordance]['concordantLimit'][limitKey].get('numIndividualsInSubsetA')

    def getnumIndividualsInSubsetB(self, spartition: str, concordance: str, limitKey: str) -> int:
        return self.getSpartitionFromLabel(spartition)['concordances'][concordance]['concordantLimit'][limitKey].get('numIndividualsInSubsetB')


    def getconcordanceSupport(self, spartition: str, concordance: str, limitKey: str) -> float:
        return self.getSpartitionFromLabel(spartition)['concordances'][concordance]['concordantLimit'][limitKey].get('concordanceSupport')
...
class SpartParserXML:
    ...
    keyMap = {
        ...
        'concordantLimit': ['concordantlimit'],
        'concordantLimits': ['concordantlimits'],
        'subsetnumberA': [],
        'subsetnumberB' : [],
    ...
    def parseSubsetIndividual(self, element, spartition, subset):
        ...
        self.spartDict['spartitions'][spartition]['subsets'][subset]['individuals'][ref] = attrs
    ...
    def parseConcordance(self, element, spartition):
        evidenceName, attrs = self.processElement(element, 'evidenceName')
        self.spartDict['spartitions'][spartition]['concordances'][evidenceName] = attrs
        self.spartDict['spartitions'][spartition]['concordances'][evidenceName]['concordantlimit'] = {}
        self.spartDict['spartitions'][spartition]['concordances'][evidenceName]['analysis'] = ''
        self.spartDict['spartitions'][spartition]['concordances'][evidenceName]['date'] = ''

        for event, element in self.tokenizer:
            token = self.translate(element.tag)
            if (event, token) == ('start', 'analysis'):
                self.parseAnalysis(element, spartition, evidenceName)
            if (event, token) == ('start', 'date'):
                self.parseConcDate(element, spartition, evidenceName)
            if (event, token) == ('end', 'concordance'):
                break
            elif (event, token) == ('start', 'concordantLimit'):
                self.parseConcordantLimit(element, spartition, evidenceName)
            element.clear()

    def parseConcordantLimit(self, element, spartition, concordance):
        subA, attrs = self.processElement(element, 'subsetnumberA')
        subB, attrs = self.processElement(element, 'subsetnumberB')
        limitKey = subA + '_' + subB
        attrs.pop('subsetnumberA')
        self.spartDict['spartitions'][spartition]['concordances'][concordance]['concordantlimit'][limitKey] = attrs

    def parseAnalysis(self, element, spartition,concordance):
        attrs = self.processElement(element, 'analysis')
        self.spartDict['spartitions'][spartition]['concordances'][concordance]['analysis'] = attrs
        element.clear()

    def parseConcDate(self, element, spartition,concordance):
        self.spartDict['spartitions'][spartition]['concordances'][concordance]['date'] = element.text
        element.clear()
        ...
        for spartion in range(1,numOfspart+1):
            ...
            if not len(subset_score_type_list) < 1:
                if subset_score_type_list[spartion-1].strip() == '?':
                    self.spartDict['spartitions'][spartionNumber]['subsetScoreType'] = None
                else:
                    self.spartDict['spartitions'][spartionNumber]['subsetScoreType'] = subset_score_type_list[spartion-1].strip()

            if not len(spartition_score_type_list) < 1:
                if spartition_score_type_list[spartion-1].strip() == '?':
                    self.spartDict['spartitions'][spartionNumber]['spartitionScoreType'] = None
                else:
                    self.spartDict['spartitions'][spartionNumber]['spartitionScoreType'] = spartition_score_type_list[spartion-1].strip()

            if not len(individual_score_type_list) < 1:
                if individual_score_type_list[spartion-1].strip() == '?':
                    self.spartDict['spartitions'][spartionNumber]['individualScoreType'] = None
                else:
                    self.spartDict['spartitions'][spartionNumber]['individualScoreType'] = individual_score_type_list[spartion-1].strip()
            ...
            if spartionLabel[-1].strip() == '?':
                ...
                self.spartDict['spartitions'][spartionNumber]['spartitionScore'] = float(spartionLabel[1].strip())
...
class SpartWriterXML:
    ...
    def writeConcordance(self, spartition: str, concordance: str):
        ...
        concordance_attributes = self.formatData(data, 'evidenceName', concordance)

        # Start the concordance element without analysis and date attributes
        concordance_filtered_attributes = {k: v for k, v in concordance_attributes.items() if k not in ['analysis', 'date']}
        self.handler.startElement('concordance', concordance_filtered_attributes)

        # Add analysis as a separate element
        self.handler.startEndElement('analysis', {'name': data['analysis']})

        # Add date as a separate element
#        self.handler.startElement('date', {})


        self.handler.startEndElement('date', characters=data['date'].isoformat())

#        datetime.datetime.now().replace(microsecond=0).isoformat()
#        self.handler.characters(data['date'])

#        self.handler.endElement('date')

        # Add concordant limits
        for concordantLimit in self.spart.getConcordantLimits(spartition, concordance):
            limitData = self.spart.getConcordantLimitData(spartition, concordance, concordantLimit)
            x = concordantLimit.split('_')
            subsetNumberA = x[0]
            subsetNumberB = x[1]
            limitData = self.formatData(limitData, 'subsetNumberB', subsetNumberB)
            limitData = self.formatData(limitData, 'subsetNumberA',subsetNumberA)
            self.handler.startEndElement('concordantLimit', limitData)
...
if __name__ == '__main__':
    spart = Spart.fromPath('SpartWriterXML')
    demoDir = Path("examples")
